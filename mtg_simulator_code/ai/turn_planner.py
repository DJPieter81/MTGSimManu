...
class CombatPlanner:
    ...
    # Tunable parameters (LLM judge can adjust these)
    MAX_ATTACK_CONFIGS = 32    # max configurations to evaluate
    LETHAL_BONUS = 101.8       # bonus for lethal attacks
    TWO_TURN_LETHAL_BONUS = 14.6  # bonus for setting up lethal next turn
    TRADE_UP_BONUS = 2.0       # bonus for killing higher-value creatures
    TRADE_DOWN_PENALTY = -4.5  # penalty for losing higher-value creatures (was -6.1)
    ...
    SHIELDS_DOWN_PENALTY = -2.5  # penalty for tapping out attackers when opp has tricks (was -4.5)
    ...
    def plan_attack(self, board: VirtualBoard) -> Tuple[List[VirtualCreature], float]:
        ...
        if total_power >= board.opp_life:
            return valid_attackers, self.LETHAL_BONUS
        ...
        for config in configs:
            ...
            # Bonus for lethal
            if result.is_lethal:
                delta += self.LETHAL_BONUS

            # Bonus for 2-turn lethal setup
            if result.two_turn_lethal:
                delta += self.TWO_TURN_LETHAL_BONUS
            ...
            if board.opp_mana >= 2:
                ...
                if tapped_value > 5.0:
                    ...
                    scaled_penalty = self.SHIELDS_DOWN_PENALTY * (1.0 - damage_ratio * 0.6)
    ...
    def _generate_attack_configs(self, attackers: List[VirtualCreature],
        ...
        return unique_configs[:self.MAX_ATTACK_CONFIGS]
...
class TurnPlanner:
    ...
    Tunable parameters (LLM judge can adjust):
    """
    # Strategic weights
    PRE_COMBAT_REMOVAL_BONUS = 2.5   # bonus for removing blockers before combat
    MANA_RESERVATION_WEIGHT = 5.2    # how much to value holding up mana
    POST_COMBAT_DEPLOY_BONUS = 0.9   # slight bonus for deploying threats after combat
    INFORMATION_BONUS = 0.3          # bonus for attacking first (see blocks before committing)

    # Response thresholds
    COUNTER_THRESHOLD = 5.5      # minimum threat value to counter
    COUNTER_CHEAP_THRESHOLD = 2.0  # threshold when counter costs ≤2 mana
    REMOVAL_RESPONSE_THRESHOLD = 5.2  # minimum value to use instant removal
    BLINK_SAVE_THRESHOLD = 3.5   # minimum creature value to blink-save
    ...
    def evaluate_response(self, board: VirtualBoard,
        ...
        for response in available_responses:
            ...
            if "counterspell" in response.tags:
                ...
                if response.cmc <= 2:
                    threshold = self.COUNTER_CHEAP_THRESHOLD
                else:
                ...
                if threat_spell and threat_spell.is_creature:
                    ...
                    if net_value < self.REMOVAL_RESPONSE_THRESHOLD:
                        continue
                ...
                if threat_spell and "removal" in threat_spell.tags:
                    ...
                    if board.my_creatures:
                        ...
                        if best_creature.value >= self.BLINK_SAVE_THRESHOLD:
        ...
        if removal and sim.opp_creatures:
            ...
            for spell in removal:
                for target in sim.opp_creatures:
                    ...
                    improvement += self.PRE_COMBAT_REMOVAL_BONUS
    ...
    def _evaluate_hold_up_mana(self, board: VirtualBoard,
        ...
        if has_counter:
            score += self.MANA_RESERVATION_WEIGHT * 1.5
            reasoning_parts.append("Hold up counter mana")
        if has_removal:
            score += self.MANA_RESERVATION_WEIGHT
...
def _spell_damage(card) -> int:
    """Extract damage amount from a spell (for burn spells)."""
    name = card.template.name.lower()
    # Common burn spells
    BURN_DAMAGE = {
        "lightning bolt": 3, "lava spike": 3, "rift bolt": 3,
        "searing blaze": 3, "galvanic blast": 4,
        "lightning helix": 3, "boros charm": 4,
        "unholy heat": 6, "fatal push": 99,  # kills anything
        "solitude": 99, "fury": 99,  # evoke removal
        "prismatic ending": 99, "march of otherworldly light": 99,
        "path to exile": 99, "swords to plowshares": 99,
    }
    for pattern, dmg in BURN_DAMAGE.items():
        if pattern in name:
            return dmg
