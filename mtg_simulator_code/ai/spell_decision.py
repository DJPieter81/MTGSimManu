...
def _apply_pre_filters(ctx: _DecisionContext) -> List["CardInstance"]:
    ...
    for card in ctx.castable:
        ...
        # EXCEPTION: protection spells (Ephemerate, Undying Evil) are allowed
        # when we have a creature pending end-of-turn exile (Goryo's Vengeance)
...
def _concern_advance(ctx: _DecisionContext) -> Optional[SpellDecision]:
    ...
    if ctx.assessment.turn_number <= 3:
        for card in ctx.castable:
...
def _advance_proactive(ctx: _DecisionContext) -> Optional[SpellDecision]:
    ...
    if ctx.my_threats:
        ...
        cmc = best.template.cmc or 0
        available = ctx.assessment.my_mana
        if cmc == available:
            reason += " (uses all mana)"
        elif cmc < available and len(ctx.my_threats) > 1:
            reason += f" (leaves {available - cmc} mana open)"
