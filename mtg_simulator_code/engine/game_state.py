...
class GameState:
    ...
    def can_cast(self, player_idx: int, card: CardInstance) -> bool:
        ...
        if template.evoke_cost is not None and total_mana < effective_cmc:
            ...
            if exile_candidates:
                ...
                can_evoke = evaluate_action(self, player_idx, Action(ActionType.EVOKE, {'card': card})) > 0
    ...
    def play_land(self, player_idx: int, card: CardInstance):
        ...
        # ── Shockland: pay 2 life if hand needs the mana this turn ──
            ...
            needs = analyze_mana_needs(self, player_idx)
            land_colors = set(card.template.produces_mana)
            # Shock if: we have spells that need this mana, or we're missing colors this land provides
            has_spells_to_cast = needs.cheapest_spell_cmc <= needs.total_mana + 1
            fixes_missing_color = bool(land_colors & needs.missing_colors)
            should_shock = has_spells_to_cast or fixes_missing_color
            if should_shock:
                player.life -= 2
                card.enter_battlefield()
                card.tapped = False
    ...
    def _crack_fetchland(self, player_idx: int, fetch_card: CardInstance):
        ...
        # ── Hand-aware fetch target selection via ManaPlanner ──
        from ai.mana_planner import analyze_mana_needs, choose_fetch_target
        needs = analyze_mana_needs(self, player_idx, player.effective_cmc_overrides)
        best_land = choose_fetch_target(player.library, fetch_colors, needs, turn=self.turn_number)

        if best_land:
            player.library.remove(best_land)
            best_land.controller = player_idx

            # Shocklands fetched: use mana_planner to decide shock
            if best_land.name in SHOCK_LANDS:
                from ai.mana_planner import analyze_mana_needs
                needs = analyze_mana_needs(self, player_idx)
                land_colors = set(best_land.template.produces_mana)
                has_spells_to_cast = needs.cheapest_spell_cmc <= needs.total_mana + 1
                fixes_missing_color = bool(land_colors & needs.missing_colors)
                should_shock = has_spells_to_cast or fixes_missing_color
                if should_shock:
    ...
    def cast_spell(self, player_idx: int, card: CardInstance,
        ...
        if not free_cast:
            ...
            if template.dash_cost is not None:
                ...
                from ai.board_eval import evaluate_action, Action, ActionType
                dashed = evaluate_action(self, player_idx, Action(ActionType.DASH, {'card': card, 'can_normal': can_normal, 'can_dash': can_dash})) > 0
            ...
            from ai.board_eval import evaluate_action as _eval_action, Action as _Action, ActionType as _AT
            _do_evoke = (
                not dashed and not escaped
                and template.evoke_cost is not None
                and untapped < template.mana_cost.cmc
                and _eval_action(self, player_idx, _Action(_AT.EVOKE, {'card': card})) > 0
            )
            should_evoke = _do_evoke
            # Last-minute target check: for cards that bounce creatures (Subtlety),
            # verify opponent still has creatures before committing to evoke
            if should_evoke:
                oracle_lower = (template.oracle_text or "").lower()
                if ('creature spell' in oracle_lower or 'target creature' in oracle_lower) and 'removal' not in (template.tags or set()):
    ...
    def _execute_spell_effects(self, item: StackItem):
        ...
        # ── Legacy fallback: Griselbrand (no-op, activated ability handled in AI) ──
        if name == "Griselbrand":
            pass
            return

        # DEAD CODE BELOW: kept temporarily for cards not yet migrated
        if False and name == "Goryo's Vengeance":
            gy = self.players[controller].graveyard
            ...
            # Also check non-legendary big creatures
            ...
            if legendary_creatures:
                ...
                self.reanimate(controller, best, exile_at_eot=True, give_haste=True)
            return

        # Persist (the card, not the keyword): reanimate without counters
        if name == "Persist":
            gy = self.players[controller].graveyard
            ...
            if creatures:
                best = max(creatures,
                           key=lambda c: (c.template.power or 0) + (c.template.toughness or 0))
                self.reanimate(controller, best)
            return

        # Unmarked Grave: tutor a nonlegendary card to graveyard
        if name == "Unmarked Grave":
            lib = self.players[controller].library
            # Find best creature to put in graveyard
            creatures = [c for c in lib if c.template.is_creature]
            if creatures:
                best = max(creatures,
                           key=lambda c: (c.template.power or 0) + (c.template.toughness or 0))
                lib.remove(best)
                best.zone = "graveyard"
                self.players[controller].graveyard.append(best)
                self.rng.shuffle(lib)
                self.log.append(f"T{self.turn_number} P{controller+1}: "
                                f"Unmarked Grave puts {best.name} in graveyard")
            return

        # Grapeshot: deal 1 damage to any target (storm handles copies)
        if name == "Grapeshot":
            self.players[opponent].life -= 1
            self.players[controller].damage_dealt_this_turn += 1
            return

        # Past in Flames: give flashback to instants/sorceries in graveyard
        if name == "Past in Flames":
            for c in self.players[controller].graveyard:
        ...
        # Empty the Warrens: create 2 goblin tokens (storm copies add more)
        if name == "Empty the Warrens":
            self.create_token(controller, "goblin", count=2)
            return

        # Galvanic Relay: exile top cards, can play next turn (simplified: draw)
        if name == "Galvanic Relay":
            # Simplified: draw cards equal to storm count
            draw_count = min(self._global_storm_count, 5)
            self.draw_cards(controller, draw_count)
            return

        # Galvanic Discharge: deal damage based on energy
        if name == "Galvanic Discharge":
            energy = self.players[controller].energy_counters
            # Spend all energy for maximum damage
            spend = min(energy, 5)
            damage = 2 + spend  # base 2 + energy spent
            if self.players[controller].spend_energy(spend):
                pass
                ...
                for tid in item.targets:
                    ...
                    if target and target.zone == "battlefield" and target.template.is_creature:
                        ...
                        if target.is_dead:
                            self._creature_dies(target)
                        return
            # Default: damage to opponent
            opp.life -= damage
            self.players[controller].damage_dealt_this_turn += damage
            return

        # Griselbrand: activated ability (draw 7, pay 7 life) handled in AI
        if name == "Griselbrand":
            # ETB: nothing special, the draw-7 is an activated ability
            pass

        # Summoner's Pact: search for green creature
        if name == "Summoner's Pact":
            ...
            if green_creatures:
                best = max(green_creatures,
                           key=lambda c: (c.template.power or 0))
                lib.remove(best)
                best.zone = "hand"
                self.players[controller].hand.append(best)
                self.rng.shuffle(lib)
            return

        # Thoughtseize: opponent reveals hand, you choose nonland to discard
        if name == "Thoughtseize":
            self.players[controller].life -= 2
            self._force_discard(opponent, 1)
            return

        # Faithful Mending: draw 2, discard 2
        if name == "Faithful Mending":
            self.draw_cards(controller, 2)
            self._force_discard(controller, 2)
            self.gain_life(controller, 2, "Faithful Mending")
            return

        # Wrath of the Skies: destroy all creatures with MV <= X (X = energy spent)
        if name == "Wrath of the Skies":
            # Spend all available energy to maximize X
            player = self.players[controller]
            x_val = min(player.energy_counters, 10)  # Spend up to 10 energy
            if x_val > 0:
                player.spend_energy(x_val)
            # Destroy all nonland permanents with MV <= X
            for p in self.players:
                to_destroy = [c for c in p.battlefield
                              if not c.template.is_land
                              and c.template.cmc <= (x_val + 2)  # Base cost is 2 + X energy
                ...
                for creature in to_destroy:
                    if creature.template.is_creature:
                        ...
                        self._permanent_destroyed(creature)
            self.log.append(f"T{self.turn_number} P{controller+1}: "
                           f"Wrath of the Skies (X={x_val}) sweeps the board")
            return

        # Prismatic Ending: exile target nonland permanent with MV <= colors spent
        if name == "Prismatic Ending":
            opp = self.players[opponent]
            # Can exile anything with CMC <= 5 (converge with 5 colors)
            player_lands = self.players[controller].lands
            colors = set()
            ...
            if max_cmc < 1:
                max_cmc = 1
            # Target best nonland permanent
            targets = [c for c in opp.battlefield
                      if not c.template.is_land and c.template.cmc <= max_cmc]
            if targets:
                target = max(targets, key=lambda c: c.template.cmc)
                self._exile_permanent(target)
                self.log.append(f"T{self.turn_number} P{controller+1}: "
                               f"Prismatic Ending exiles {target.name}")
            return

        # Stock Up: draw 2 cards
        if name == "Stock Up":
            self.draw_cards(controller, 2)
            return

        # Orim's Chant: opponent can't cast spells this turn (simplified: skip)
        if name == "Orim's Chant":
            # Simplified: prevent opponent from casting spells this turn
            self.players[opponent].silenced_this_turn = True
            self.log.append(f"T{self.turn_number} P{controller+1}: "
                           f"Orim's Chant silences opponent")
            return

        # Galvanic Discharge: deal X damage where X = 2 + energy spent
        if name == "Galvanic Discharge":
            player = self.players[controller]
            ...
            if hasattr(item, 'targets') and item.targets:
                for tid in item.targets:
                    if tid == -1:
                        # AI chose to go face
                        break
                    candidate = self.get_card_by_id(tid)
            ...
            if target is None and (not hasattr(item, 'targets') or not item.targets or (item.targets and item.targets[0] != -1)):
                killable = [
                    c for c in opp.creatures
                    if damage >= (c.toughness or 0) - (getattr(c, 'damage_marked', 0) or 0)
                ]
                if killable:
                    target = max(killable, key=lambda c: c.power or 0)
            if target:
                target.damage_marked += damage
                self.log.append(f"T{self.turn_number} P{controller+1}: "
                               f"Galvanic Discharge deals {damage} to {target.name}")
                if target.is_dead:
                    self._creature_dies(target)
            else:
                opp.life -= damage
        ...
        # Ephemerate: exile target creature you control, return it
        if name == "Ephemerate":
            my_creatures = self.players[controller].creatures
            if my_creatures:
                # Blink best creature for ETB value
                best = max(my_creatures,
                           key=lambda c: c.template.cmc)
                self._blink_permanent(best, controller)
            return

        # Undying Evil: target creature gains undying
        if name == "Undying Evil":
            # Simplified: give target creature undying until eot
            my_creatures = self.players[controller].creatures
            if my_creatures:
        ...
        # Wish: get a card from sideboard (or library as fallback)
        if name == "Wish":
            player = self.players[controller]
            sb = player.sideboard
            lib = player.library
            ...
            # Priority: search sideboard first (real Wish behavior)
            if sb:
                # Look for storm finishers in sideboard (prioritize Grapeshot for lethal)
                ...
                for fname in finisher_priority:
                    ...
                    # Any instant/sorcery from sideboard
            ...
            # Fallback: search library if sideboard is empty or has no targets
            if not chosen:
                finisher_priority_lib = ["Grapeshot", "Empty the Warrens", "Galvanic Relay"]
                for fname in finisher_priority_lib:
            ...
            if chosen:
                ...
                self.log.append(f"T{self.turn_number} P{controller+1}: Wish finds {chosen.name} (from {from_zone})")
            return

        # Gifts Ungiven: search for 4 cards, opponent puts 2 in GY, 2 in hand
        # Simplified: tutor for 2 cards (best instants/sorceries)
        if name == "Gifts Ungiven":
            lib = self.players[controller].library
            ...
            for card_found in found:
                ...
                # Gifts puts 2 in GY (opponent chooses), but we want the key ones
                card_found.zone = "graveyard"
                self.players[controller].graveyard.append(card_found)
            self.rng.shuffle(lib)
            if found:
                self.log.append(f"T{self.turn_number} P{controller+1}: "
                ...
                for tid in item.targets:
                    ...
                    if target and target.zone == "battlefield" and target.template.is_creature:
                        ...
                        if target.is_dead:
                            self._creature_dies(target)
                        return
            self.players[opponent].life -= 3
            self.players[controller].damage_dealt_this_turn += 3
            return

        # Lava Dart: deal 1 damage (has flashback: sacrifice a Mountain)
        if name == "Lava Dart":
            if item.targets:
                for tid in item.targets:
                    target = self.get_card_by_id(tid)
        ...
        # Unholy Heat: deal 2 damage (6 with delirium)
        if name == "Unholy Heat":
            # Simplified delirium: check if 4+ card types in graveyard
            gy = self.players[controller].graveyard
            types_in_gy = set()
            for c in gy:
        ...
        if name == "Mutagenic Growth":
            ...
            # Pay 2 life (Phyrexian mana)
            self.players[controller].life -= 2
            return

        # Violent Urge: target creature gets +1/+0 and gains first strike, draw a card
        if name == "Violent Urge":
            my_creatures = self.players[controller].creatures
            if my_creatures:
                best = max(my_creatures, key=lambda c: c.power or 0)
        ...
        # Expressive Iteration: look at top 3, exile 1, put 1 in hand
        if name == "Expressive Iteration":
            self.draw_cards(controller, 1)  # Simplified: draw 1 card
            return
        ...
        # Sleight of Hand: look at top 2, put 1 in hand (simplified: draw 1)
        if name == "Sleight of Hand":
            self.draw_cards(controller, 1)
            return

        # Reckless Impulse: exile top 2, play until end of next turn (simplified: draw 2)
        if name == "Reckless Impulse":
            self.draw_cards(controller, 2)
            return

        # Wrenn's Resolve: exile top 2, play until end of next turn (simplified: draw 2)
        if name == "Wrenn's Resolve":
            self.draw_cards(controller, 2)
            return

        # Heroes' Hangout: exile top 2, choose 1 to play (simplified: draw 1)
        if name == "Heroes' Hangout":
            self.draw_cards(controller, 1)
            return

        # Tribal Flames: deal damage equal to domain (number of basic land types)
        if name == "Tribal Flames":
            player = self.players[controller]
            land_types = set()
            for c in player.battlefield:
            ...
            if damage < 2:
                damage = 2  # minimum from 2 colors
            self.players[opponent].life -= damage
            self.players[controller].damage_dealt_this_turn += damage
            return
