...
def galvanic_discharge_resolve(game, card, controller, targets=None, item=None):
    ...
    # Use AI-chosen targets if available (creature OR planeswalker —
    # Galvanic Discharge oracle: "target creature or planeswalker").
    chosen_target = None
    if targets:
        for tid in targets:
            if tid == -1:
            ...
            if (candidate and candidate.zone == "battlefield"
                    and (candidate.template.is_creature
                         or CardType.PLANESWALKER in candidate.template.card_types)):
                chosen_target = candidate
                break
    # Fallback: pick best killable creature (not just highest power)
    if chosen_target is None and (not targets or (targets and targets[0] != -1)):
        killable = [
            c for c in opp.creatures
            if damage >= (c.toughness or 0) - (getattr(c, 'damage_marked', 0) or 0)
        ]
        if killable:
            chosen_target = max(killable, key=lambda c: c.power or 0)
    if chosen_target:
        deal_damage(game, chosen_target, damage, source_controller=controller)
        game.log.append(f"T{game.display_turn} P{controller+1}: "
                        f"Galvanic Discharge deals {damage} to {chosen_target.name}")
    else:
        deal_damage(game, opp, damage, source_controller=controller)
        game.log.append(f"T{game.display_turn} P{controller+1}: "
