...
def position_value(snap: "EVSnapshot") -> float:
    ...
    if my_clock >= NO_CLOCK and opp_clock >= NO_CLOCK:
        ...
        #
        # This is deliberately the SIGN half alone. The sentinel-cliff
        # half (one saturating form for the finite branch too, so a
        # real-but-slow clock never scores below no clock) was built and
        # A/B-measured on 2026-09-04: Creatures Toolbox 17.5% -> 11.7%,
        # Jeskai Blink 30.8% -> 27.5% (n=5 field, same seeds). It repeats
        # the 2026-08-30 finding that the combined fix costs win rate
        # without a benefit, so it stays out. See
