...
| `ETB_RETURN_FROM_GRAVEYARD_TO_HAND` | When the permanent enters, its trigger returns a target card from the controller's graveyard to hand. The target may be filtered ("target instant or sorcery card", "target creature card", "target enchantment card", "target permanent card", etc.) or unrestricted ("target card"). Optional ("you may return") still counts. Do NOT apply to triggers that are conditional on kicker, descend, attack triggers, leaves-the-battlefield, or non-ETB activations — those are different mechanisms. |
