...
class EVPlayer:
    ...
    def _score_spell(self, card: "CardInstance", snap: EVSnapshot,
        ...
        # ── PROTOTYPE: Artifact-scaling equipment EV overlay ──
        # TODO(prototype/card_ev_overrides): Replace with data-driven
        # card_ev_overrides JSON subsystem. Currently hard-wired to test
        # the Plating/Nettlecyst hypothesis: does the vs-Affinity gap
        # close when equipment EV reflects artifact-count scaling?
        #
        # Oracle-driven: matches "equipped creature gets +X/+0 ... for each
        # artifact you control" (Cranial Plating, Nettlecyst, Cranial Ram).
        # The baseline projection treats this as a plain 2-mana artifact
        # with EV ~0. Real value is: artifact_count × expected-connect-turns
        # × swing-impact. Signal Pest + equipment = most games close T5.
        #
        # TUNING v2: initial v1 formula (burn_face_mult * 0.6) made Affinity
        # field WR jump +3.6pp because it over-credited Plating without
        # accounting for (a) equip-turn delay — Plating needs 1 to cast + 3
        # to equip, usually across 2 turns; (b) connect probability — only
        # ~30% of equipped swings actually land full damage vs competent
        # blockers; (c) single-creature fragility — all value dies to 1
        # removal spell. v2 applies all three discounts multiplicatively.
        #
        # Scope: ONLY fires for equipment whose oracle matches the pattern.
        # No hardcoded names. Prototype kept minimal; if it shifts the WR,
        # we move to proper card_ev_overrides infra and retire this block.
        if ('equipment' in tags
                and 'for each artifact you control' in t_oracle
                and re.search(r'gets?\s+\+\d+/\+\d+', t_oracle)):
            # Count artifacts already in play (our battlefield).
            from engine.cards import CardType
            artifact_count = sum(
                1 for c in me.battlefield
                if CardType.ARTIFACT in c.template.card_types
            )
            # Plating also counts *itself* on resolution, so +1.
            artifact_count += 1
            carriers = [c for c in me.creatures if (c.power or 0) >= 0]
            if carriers:
                best_carrier_power = max(c.power or 0 for c in carriers)
                expected_dmg = best_carrier_power + artifact_count
                # TUNING v2 — three discounts:
                # 1. CONNECT_RATE: empirical ~30% vs blocking opp, ~80% vs open
                #    board. Prior was 50%/100%.
                if snap.opp_creature_count == 0:
                    connect_rate = 0.8
                else:
                    connect_rate = 0.3
                # 2. EQUIP_TURN_DELAY: Plating equip cost = 3 mana + untap.
                #    If we cast Plating this turn, we equip next turn at
                #    earliest. Discount by ~0.6 for the time-value.
                #    Exception: we have leftover mana post-cast AND a carrier.
                equip_cost = getattr(t, 'equip_cost', None) or 3
                mana_after_cast = max(0, snap.my_mana - (t.cmc or 0))
                can_equip_same_turn = mana_after_cast >= equip_cost
                timing_discount = 1.0 if can_equip_same_turn else 0.6
                # 3. REMOVAL_FRAGILITY: one removal kills the whole plan.
                #    Estimate: opponent has ~20% chance per turn to draw
                #    answer before we swing. Over 2 turns of latency,
                #    ~36% of the time the plan dies before paying off.
                #    Apply survival multiplier = 0.64.
                fragility_discount = 0.64
                projected_dmg = (expected_dmg
                                 * connect_rate
                                 * timing_discount
                                 * fragility_discount)
                # Convert damage to EV via burn_face_mult (profile-driven).
                # No additional 0.6 amortisation — that was the v1 mistake.
                ev += projected_dmg * p.burn_face_mult
            else:
                # No carrier yet. Hold-bonus floor, kept small so we don't
                # deploy Plating into empty board pre-curveout.
                ev += 1.0
