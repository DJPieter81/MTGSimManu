...
def classify_log(raw: str) -> dict:
    ...
    if body.startswith("Cast "):
        ...
        if "(Dash)" in body:
            det["dash"] = True
        # Extract mana cost from parentheses, e.g. "Cast Lightning Bolt (R)"
        cost_match = re.search(r'\(([WUBRGC0-9/{}]+)\)', body)
        if cost_match:
            det["mana_cost"] = cost_match.group(1)
    ...
    # ── Combat damage (detailed) ─────────────────────────────
    # Attacker/blocker combat damage: "Ragavan (2/1) deals 2 combat damage to P2 (life: 18)"
    cm = re.search(r'(.+?)\s+\((\d+)/(\d+)\)\s+deals\s+(\d+)\s+combat damage to\s+(.+)', body)
    if cm:
        attacker_name = cm.group(1)
        power, toughness = int(cm.group(2)), int(cm.group(3))
        damage = int(cm.group(4))
        target = cm.group(5).strip()
        # Check if target is a player (P1/P2) or a creature
        pm = re.match(r'P(\d+)\s*\(life:\s*(\d+)\)', target)
        if pm:
            return _evt("combat", "combat_damage_player", player, attacker_name, body, raw,
                         {"damage": damage, "power": power, "toughness": toughness,
                          "target_player": int(pm.group(1)) - 1,
                          "remaining_life": int(pm.group(2))})
        else:
            # Damage to a creature (blocker)
            tm = re.match(r'(.+?)\s+\((\d+)/(\d+)\)', target)
            if tm:
                return _evt("combat", "combat_damage_creature", player, attacker_name, body, raw,
                             {"damage": damage, "power": power, "toughness": toughness,
                              "target_creature": tm.group(1),
                              "target_power": int(tm.group(2)), "target_toughness": int(tm.group(3))})
            return _evt("combat", "combat_damage_creature", player, attacker_name, body, raw,
                         {"damage": damage, "target_creature": target})

    # Trample: "Primeval Titan tramples 3 damage through to P2 (life: 12)"
    trm = re.search(r'(.+?)\s+tramples\s+(\d+)\s+damage through to\s+P(\d+)\s*\(life:\s*(\d+)\)', body)
    if trm:
        return _evt("combat", "trample", player, trm.group(1), body, raw,
                     {"damage": int(trm.group(2)),
                      "target_player": int(trm.group(3)) - 1,
                      "remaining_life": int(trm.group(4))})

    # Lifelink: "Solitude lifelink gains 3 life (life: 23)"
    llm = re.search(r'(.+?)\s+lifelink gains\s+(\d+)\s+life\s*\(life:\s*(\d+)\)', body)
    if llm:
        return _evt("combat", "lifelink", player, llm.group(1), body, raw,
                     {"life_gained": int(llm.group(2)),
                      "new_life": int(llm.group(3))})

    # Deathtouch: "Ragavan deathtouch kills Blocker"
    dtm = re.search(r'(.+?)\s+deathtouch kills\s+(.+)', body)
    if dtm:
        return _evt("combat", "deathtouch", None, dtm.group(1), body, raw,
                     {"killed": dtm.group(2)})
...
class ReplayGenerator:
    ...
    def snapshot(self, game, names, event_text="", extra_events=None):
        ...
        # Drain strategic annotations into the snapshot
        strategic = []
        if hasattr(self, '_strategic_logger') and self._strategic_logger:
            strategic = self._strategic_logger.drain()
        ...
        s = {
            ...
            "event": event_text,
            "events": all_events,
            "strategic_annotations": strategic,
    ...
    def run(self, d1_name, d1_list, d2_name, d2_list, d1_sideboard=None, d2_sideboard=None):
        self.snapshots = []
        self._log_cursor = 0
        ...
        if d1_sideboard:
            for template in self.build_deck(d1_sideboard):
                card = CardInstance(
        ...
        if d2_sideboard:
            for template in self.build_deck(d2_sideboard):
                card = CardInstance(
        ...
        # Wire up strategic logger for rich decision annotations
        self._strategic_logger = StrategicLogger()
        ai1.strategic_logger = self._strategic_logger
        ai2.strategic_logger = self._strategic_logger
        # Also attach to game so evaluator.assess_role can find it
        game._strategic_logger = self._strategic_logger
        # Wire logger into GoalEngines
        if ai1.goal_engine:
            ai1.goal_engine.strategic_logger = self._strategic_logger
        if ai2.goal_engine:
            ai2.goal_engine.strategic_logger = self._strategic_logger
