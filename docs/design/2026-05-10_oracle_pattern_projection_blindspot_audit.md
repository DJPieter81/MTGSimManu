...
| Tutor "into your hand" search | `ai/ev_evaluator.py:1273-1278` (`_is_real_dig`) | `'search your library'` | boolean | This one is a *gate* not a projection — but the projection that calls it (signal #6 tutor branch) doesn't differentiate "tutor 1 card" from "tutor 2/3 cards" oracle phrasings. Fetch lands miss this entirely (different mechanic), but Mastermind's Acquisition / Bring to Light / Tinker variants would. |
