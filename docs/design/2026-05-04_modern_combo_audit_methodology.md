...
| **H. Bo1 hate-card density** *(round-3)* | `decks/lists/*.txt` mainboard counts | Mainboard count too low for the sim's Bo1 default. 2-of = ~22% in opener; matchup-hinge hate cards need **3-of minimum** for the sim to evaluate the matchup faithfully (matrix is largely G1/Bo1; Bo3 only via `--bo3`). |
...
| Affinity (overperformance) | 84.9% | ~55% | 30pp | **Inverted H** — every opponent's hate-card count vs Affinity is too low for Bo1 (single Wear // Tear ≈ 7% in opener) |
...
8. **Bo1 hate-card density:** for each combo / control matchup the deck cares about, count the matchup-specific hate card in `decks/lists/<deck>.txt`'s mainboard. **3-of is the minimum** for the sim's G1/Bo1 default to faithfully evaluate the matchup. Bump 2→3 if the matchup hinges on the card. Legacy: +10pp on `wst vs burn` after Sanctifier en-Vec 2→3.
...
The truly architectural Modern gap is most likely **Affinity overperformance**
— every opponent runs too few maindeck artifact-hate cards for Bo1, and the
fix is on opponents' decklists, not in the AI scoring layer.
